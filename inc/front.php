...
class SEO_Auto_Linker_Front extends SEO_Auto_Linker_Base
{
     ...
     * @since 0.7
     */
    public static function get_meta($post, $default = '')
    {
        $res = apply_filters('seoal_pre_get_meta', false, $default, $post);
        if ($res !== false) {
            return $res;
        }
        if (isset($post->ID)) {
            $res = get_post_meta($post->ID, self::get_key($default), true);
        } else {
            $res = '';
        }
        return $res;
